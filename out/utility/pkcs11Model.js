...
const digestModel_1 = require("./digestModel");
class Pkcs11Model extends vscode_1.Disposable {
    ...
    async generateAesKey(node) {
        ...
        try {
            ...
            const isSensitive = await interactions_1.Interactions.showYesNoQuickPick("Sensitive? (default=no)");
            if (isSensitive == undefined) {
                return;
            }
            const isExtractable = await interactions_1.Interactions.showYesNoQuickPick("Extractable?");
    ...
    async generateRsaKey(node) {
        ...
        try {
            ...
            let pulicExponentString = await interactions_1.Interactions.showQuickPick(definitions_1.Definitions.SupportedRsaPublicExponents, "RSA public exponent");
            if (pulicExponentString == "custom...") {
                pulicExponentString = await interactions_1.Interactions.showInputBox("Public Exponent (integer between 3 and 65537).");
            }
    ...
    async createDigest(node) {
        const session = await this.openSession(node);
        try {
            const digest = new digestModel_1.DigestModel(session);
    ...
    async exportCertificate(node) {
        const dataType = await interactions_1.Interactions.showQuickPick(definitions_1.Definitions.SupportedCertificateOutput, "Output data type");
        if (dataType == undefined) {
            return;
        }
        try {
            const value = node.object.getAttribute("value");
            if (dataType == "pem") {
                const base64Cert = new bufferWithEncoding_1.BufferWithEncoding(value, "base64").toString();
                ...
                this.outputChannel.printBufferWithEncoding(new bufferWithEncoding_1.BufferWithEncoding(value, dataType));
            }
        }
        catch (err) {
            interactions_1.Interactions.showErrorMessage(`Failed to export certificate ${node.label}.\n${err}`);
        }
    }
    async exportDataObject(node) {
        const dataType = await interactions_1.Interactions.showQuickPick(definitions_1.Definitions.SupportedEncodingOutput, "Output data type");
        if (dataType == undefined) {
            return;
        }
        try {
            ...
            interactions_1.Interactions.showErrorMessage(`Failed to export object ${node.label}.\n${err}`);
        }
    }
    async exportPublicKey(node) {
        const keyType = node.object.getAttribute("keyType");
        const key = node.object.toType();
        try {
            if (keyType == graphene.KeyType.RSA) {
                const result = node.object.getAttribute({ modulus: null, publicExponent: null });
                const modulus = result.modulus;
                const publicExponent = result.publicExponent;
                const publicKey = {
            ...
            interactions_1.Interactions.showErrorMessage(`Failed to export public key ${node.label}.\n${err}`);
        }
    }
    async exportSecretKey(node) {
        const dataType = await interactions_1.Interactions.showQuickPick(definitions_1.Definitions.SupportedEncodingOutput, "Output data type");
        if (dataType == undefined) {
            return;
        }
        try {
            ...
            interactions_1.Interactions.showErrorMessage(`Failed to export secret key ${node.label}.\n${err}`);
        }
    }
}
