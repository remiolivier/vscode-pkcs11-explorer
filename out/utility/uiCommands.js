...
class Interactions {
    ...
    static async showInputBoxWithQuickPickType(inputTypePrompt, prompt) {
        const dataType = await UiCommands.showQuickPick(definitions_1.Definitions.SupportedSignEncodingInputData, inputTypePrompt);
        if (dataType == undefined) {
            return;
        }
        const data = await UiCommands.showInputBox(`${prompt} in ${dataType} format.`);
    ...
    async getSignVerifyData(keyType) {
        let mechanisms;
        ...
        if (keyType == graphene.KeyType.RSA) {
            ...
            signingMechanism = await UiCommands.showQuickPick(mechanisms, "RSA mechanism.");
            questionAsked = true;
            ...
            signingMechanism = await UiCommands.showQuickPick(mechanisms, "ECC mechanism.");
            questionAsked = true;
        ...
        if (!signingMechanism || signingMechanism == "Custom") {
            ...
            signingMechanism = await UiCommands.showQuickPick(mechanisms, "Mechanism.");
        }
